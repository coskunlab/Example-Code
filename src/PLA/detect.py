...
class PLA_detection(object):
    def __init__(self, path, name='imgs', m='Marker'):
        self.path = path
        self.name = name
        self.m = m
        self.get_markers()
    ...
    def detect_spot(self, marker, thres=.05, min_radius=1, max_radius=4):
        ...
        img = img[np.newaxis, ...]
        # img = self.contrast_str(img, n_min=10, n_max=100)

        print(f'Processing image {marker}')
        # White top hat filtering
        img_wth = self.white_tophat(img, 5)
        ...
        img_spot = self.plot_spot_on_image(img_wth, spots_filtered, 1, 3) 
    ...
    def get_image_from_marker(self, marker, ):
        ...
        with h5py.File(self.path, "r") as f:
            img = f[self.name][indice]
        return util.img_as_ubyte(img)
    
    def get_markers(self,):
        with h5py.File(self.path, "r") as f:
            markers = f[self.name].attrs[self.m]
...
class PLA_detection_3D(object):
    ...
    def detect_spot(self, marker, thres=.05, min_radius=1, max_radius=4):
        ...
        img_spot = self.plot_spot_on_image(imgs_wth, spots_filtered, 1, 3) 
        print(np.unique(img_spot[0], return_counts=True))
        self.PPI_dict[marker] = spots_filtered
